...
function setupWebSocket(lobbyId, cursor, wordDiv, calc) {
    ...
    ws.onmessage = function (event) {
      ...
      if (message.type == "state"){
        if (message.state == "finish"){
          endGame(calc.calculateWPM().wpm,"Lost")
        }
      }
...
function sendMessage(ws, wpm, index) {
  const message = {
    type: "message",
    clientid: clientid,
    wpm: wpm, 
...
function updateEnemyCursorPosition(cursor,wordDiv, index) {
  if (index != 0){
    let prevChar = wordDiv.children[index-1]
    if (!prevChar.classList.contains("green")){
      for (let i = index; i>=0; i--){
        prevChar = wordDiv.children[i-1]
        if (!prevChar.classList.contains("grey")&&!prevChar.classList.contains("green")){
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const ws = setupWebSocket(lobbyId, enemycursor, wordDiv, calc)
  ...
  updateEnemyCursorPosition(enemycursor, wordDiv, 0)
  ...
  function handleKeyDown(event){
    if (event.key === dbwords[index]) {
      ...
      wordDiv.children[index].classList.remove("grey");
